...
coerce_numeric_df <- function(df) {
  out <- lapply(df, function(col) {
    ...
    col_chr[col_chr %in% c("", "NA", "NaN", "NULL", "null")] <- NA
...
rownames(df_disc) <- sanitize_row_ids(df_disc$Sample_ID, prefix = "sample_id")
df_disc$Sample_ID <- NULL

# Validation set (Sheet B)
df_val <- as.data.frame(read_excel(file.path(data_path, "Xue_tableS4.xlsx"), sheet = "B"))
rownames(df_val) <- sanitize_row_ids(df_val$Sample_ID, prefix = "sample_id")
...
# Ensure feature blocks are numeric with proper NA handling
d_prot_disc <- coerce_numeric_df(d_prot_disc)
d_prot_val  <- coerce_numeric_df(d_prot_val)

# Force labels to character so category matching is stable
d_cat_disc[[label_col]] <- as.character(d_cat_disc[[label_col]])
d_cat_val[[label_col]]  <- as.character(d_cat_val[[label_col]])
