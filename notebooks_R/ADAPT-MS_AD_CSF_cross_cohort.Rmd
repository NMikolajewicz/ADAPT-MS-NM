...
rownames(prot_train) <- sapply(rownames(prot_train), function(x) {
  parts <- strsplit(x, "/")[[1]]
  sub("\\..*$", "", parts[length(parts)])
})
rownames(prot_train) <- sanitize_row_ids(rownames(prot_train), prefix = "sample")
...
rownames(prot_val) <- sapply(rownames(prot_val), function(x) {
  parts <- strsplit(x, "/")[[1]]
  sub("\\..*$", "", parts[length(parts)])
})
rownames(prot_val) <- sanitize_row_ids(rownames(prot_val), prefix = "sample")
...
rownames(d_groups) <- sanitize_row_ids(d_groups$`sample name`, prefix = "sample_name")
