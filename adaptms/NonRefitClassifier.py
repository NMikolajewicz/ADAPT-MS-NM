...
class NonRefitClassifier:
    def __init__(self, prot_df, cat_df, gene_dict, between=None):
        ...
        self.selected_features = []  # To store selected features
        self.between = between  

    def classify_and_plot(self, category1, category2, n_runs=10):
        ...
        for i in range(n_runs):
            ...
            p_values = [ttest_ind(group1[:, j], group2[:, j], nan_policy='omit')[1] for j in range(X_train.shape[1])]

            # Correct for multiple testing using FDR
            _, corrected_p_values, _, _ = multipletests(p_values, method='fdr_bh')

            # Select the top 50 features by p-value
            top_features = np.argsort(corrected_p_values)[:50]
            self.selected_features.append(top_features)
    ...
    def validate_and_plot(self, validation_prot_df, validation_cat_df, category1, category2):
        ...
        for model, top_features in zip(self.models, self.selected_features):
